            ...
            <template #header-extra>
              <span :class="['mr-4', 'rounded-8', 'p-2', 'pb-1', 'z-50', 'cursor-default', {'hover:bg-gray-700': store.darkTheme, 'hover:bg-gray-200': !store.darkTheme }]" @click.stop="()=>{}" v-if="store.activeVal === 2">
                ...
                <n-tooltip v-if="store.loginBizUser" placement="left" trigger="hover">
                  <template #trigger>
                    <n-icon class="mr-5">
                      <IIconParkOutlineExcel class="text-18px" @click.stop="handleExportExcel"/>
                    </n-icon>
                  </template>
                  导出表格
                </n-tooltip>
...
import { handleFile, saveFile } from '../hooks/fileHooks'
...
// 
  ...
  todoRef.value?.expotExcelCallback((params: Record<string, any>) => {    
    ...
    const getBlobData = async () => {
      try {
        const response = await fetch<Blob>(`${VITE_APP_API_URL}${exportExcelUrl}`, {
          method: 'POST',
          headers: { 'biz-user': store.loginBizUser || '' },
          body: Body.json(params),
          responseType: 3
        })
        const { fileName, fileType } = handleFile(response.headers['content-disposition'])
        saveFile(response.data, fileName, [fileType])
      } catch (err) {
        throw err
      }
    }
